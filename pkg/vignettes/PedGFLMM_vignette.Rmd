...
This function carries out a region-based association test using our 'beta smooth only' generalized functional linear mixed model (GFLMM), where the genetic effect function is assumed to be continuous/smooth. For details, see Jiang et al (2020). 

The genetic effect function can be expanded using either B-spline or Fourier basis functions, and the order of the basis functions needs to be specified by the user.  For small sample size datasets, one may have problems of convergence. Then, one can use a smaller number of basis functions, e.g., `betabasis_Bsp` = 6, etc.
